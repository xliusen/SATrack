...
class STARKProcessing(BaseProcessing):
    ...
    def __init__(self, search_area_factor, output_sz, center_jitter_factor, scale_jitter_factor,
        ...
        #####################################################
        self.bert_tokenizer = BertTokenizer.from_pretrained('/data/data6T/xls/projects/SeamTrack-kd/bert-base-uncased', do_lower_case=True)
    ...
    def __call__(self, data: TensorDict):
        ...
        for s in ['template', 'search']:
            ...
            # ==========================================================================================
            if s == "search":
                crops, boxes, att_mask, mask_crops = prutils.jittered_center_crop(data[s + '_images'],
                                                                                  jittered_anno,
                                                                                  data[s + '_anno'],
                ...
                crops, boxes, att_mask, mask_crops = prutils.jittered_center_crop_multi_scale(
                    data[s + '_images'],
                    jittered_anno,
                    data[s + '_anno'],
        ...
        if data['language'] == [] or data['language'] is None:
            ...
            phrase_attnmask = torch.cat(
                [phrase_attnmask, torch.zeros(nlp_len - phrase_attnmask.size(0), dtype=torch.long)], dim=0)
