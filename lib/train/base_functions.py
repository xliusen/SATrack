...
# datasets related
from lib.train.dataset import Lasot, Got10k, MSCOCOSeq, ImagenetVID, TrackingNet, TNL2K, TNL2Ktest, OTB99, OTB99LANG, WebUAV3M, VisualGenome, RefCOCOSeq
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["LASOT", "GOT10K_vottrain", "GOT10K_votval", "GOT10K_train_full", "GOT10K_official_val",
                        "COCO17", "VID", "TRACKINGNET", "VisualGenome_train", "REFCOCOG",
                        "TNL2K", "TNL2K_test", "OTB99L_train","OTB99", "OTB99_test", "WebUAV3M_train",
                        ]
        if name == "OTB99":
        ...
        if name == "TNL2K_train":
            if settings.use_lmdb:
                print("Building TNL2K_train dataset from lmdb")
        ...
        if name == "OTB99L_train":
            if settings.use_lmdb:
                print("Building OTB99L_train dataset from lmdb")
                datasets.append(OTB99LANG(settings.env.otb99lang_dir, split='train', image_loader=image_loader))
            else:
                datasets.append(OTB99LANG(settings.env.otb99lang_dir, split='train', image_loader=image_loader))

        if name == "WebUAV3M_train":
            if settings.use_lmdb:
                print("Building WebUAV3M_train dataset from lmdb")
                datasets.append(WebUAV3M(settings.env.webuav3m_dir, split='train', image_loader=image_loader))
            else:
                datasets.append(WebUAV3M(settings.env.webuav3m_dir, split='train', image_loader=image_loader))

        if name == "VisualGenome_train":
            if settings.use_lmdb:
                print("Building VisualGenome_train dataset from lmdb")
        ...
        if name == "REFCOCOG":
            if settings.use_lmdb:
                print("Building REFCOCOG 2014 google from lmdb")
    ...
    if train_cls:
        ...
        if is_main_process():
            ...
            for n, p in net.named_parameters():
                if p.requires_grad:
                    print(n, p.numel())
                    Total_Parameters += p.numel()   # 179,695,749
                    # if ("language_proj" in n) or ("language_xz_proj" in n)\
                    #         or ("vision_x_proj" in n) or ("vision_z_proj" in n):
                    #     pass
                    # else:
                    #     print(n, p.numel())
                    #     Total_Parameters += p.numel()   # 115,993,221
            print("Total Learnable parameters:", Total_Parameters)
