...
class VisionTransformerCE(VisionTransformer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        # self.multimodal_alignment = VLAlignmentModule(dim=embed_dim, num_heads=12)
    ...
    def forward_features(self, z, x, language_embeddings,
        ...
        x = self.patch_embed(x)  # b*3*256*256->b*256*768

        z_patch_embed_list=[]
        for i in range(len_z):
            z_patch_embed_list.append(self.patch_embed(z[i]))
        z = z_patch_embed_list

        vision_x_vectors = x
        vision_z_vectors_list = z
        language_vectors = language_embeddings
        ...
        if self.add_cls_token:  # 时间令牌track_query 在注意力机制前进行处理并加入到特征中
        ...
        if len_z >= 1:  # multiple memory frames 多张模板图像，需要进行展平
            z = torch.stack(z, dim=1)  # torch.Size([b, len_z, 64, 768])
            z = z.flatten(1, 2)  # b*len_z*64*768 -> b*(len_z*64))*768

        z = combine_tokens(z, language_embeddings, mode=self.cat_mode)  # (B, z+l, 768) 语言和模板共同作为后续模板来进行操作
        # ce_template_mask = torch.cat([ce_template_mask, torch.ones(B, 1, dtype=torch.bool, device=ce_template_mask.device)], dim=1)  # 拼接语言 token 的 mask

        lens_z = z.shape[1]  # HW
        lens_x = x.shape[1]  # HW

        x = combine_tokens(z, x, mode=self.cat_mode)  # (B, z(z+l)+x, 768)
        if self.add_cls_token:
            x = torch.cat([query, x], dim=1)  # (B, 1+z(z+l)+x, 768) 拼接时间令牌
        ...
        if self.add_cls_token:  # 之前拼接了,现在是分开
            query = x[:, :query_len]
        ...
        x = recover_tokens(x, lens_z_new, lens_x, mode=self.cat_mode)  # 恢复令牌
        ...
        aux_dict = {
            ...
            "vision_z_vectors": vision_z_vectors_list,
            "attn": attn,
    ...
    def forward(self, z, x, language_embeddings, ce_template_mask=None, ce_keep_rate=None,
        ...
        x, aux_dict = self.forward_features(z, x, language_embeddings, ce_template_mask=ce_template_mask,
