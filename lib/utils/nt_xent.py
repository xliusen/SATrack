...
class NTXentLoss(torch.nn.Module):
    ...
    def __init__(self, device, batch_size, temperature, use_cosine_similarity):
        ...
        self.criterion = torch.nn.CrossEntropyLoss(
            reduction="sum")  # sum all 2N terms of loss instead of getting mean val
    ...
    def _get_correlated_mask(self):
        ...
        l1 = np.eye((2 * self.batch_size), 2 * self.batch_size,
                    k=-self.batch_size)  # lower diagonal matrix, N non-zero elements
        l2 = np.eye((2 * self.batch_size), 2 * self.batch_size,
                    k=self.batch_size)  # upper diagonal matrix, N non-zero elements
    ...
    def _dot_simililarity(x, y):
        v = torch.tensordot(x.unsqueeze(1), y.T.unsqueeze(0),
                            dims=2)  # extend the dimensions before calculating similarity
        # x shape: (N, 1, C)
        # y shape: (1, C, 2N)
        # v shape: (N, 2N)
        return v
    ...
    def _cosine_simililarity(self, x, y):
        ...
        # v shape: (N, 2N)
        v = self._cosine_similarity(x.unsqueeze(1),
                                    y.unsqueeze(0))  # extend the dimensions before calculating similarity
        return v
    ...
    def forward(self, zis, zjs):
        ...
        if zjs.shape[0] != self.batch_size:
            # zjs_list = [zjs, zjs, zjs]
            len = zis.shape[0] / zjs.shape[0]
            zjs_list=[]
            for i in range(int(len)):
                zjs_list.append(zjs)
            zjs = torch.stack(zjs_list, dim=1)
            zjs = zjs.flatten(0, 1)
        ...
        l_pos = torch.diag(similarity_matrix,
                           self.batch_size)  # upper diagonal, N x [left, right] positive sample pairs
        r_pos = torch.diag(similarity_matrix,
                           -self.batch_size)  # lower diagonal, N x [right, left] positive sample pairs
        ...
        logits = torch.cat((positives, negatives),
                           dim=1)  # [2N, 2N+1], the 2N+1 elements of one column are used for one loss term
        ...
        return loss / (
                2 * self.batch_size)  # Don't know why it is divided by 2N, the CELoss can set directly to reduction='mean'
