...
def evaluate_vit(model, template, search, language):
    '''Speed Test'''
    macs1, params1 = profile(model, inputs=(template, search, language),
    ...
    with torch.no_grad():
        # overall
        for i in range(T_w):
            _ = model(template, search, language)
        start = time.time()
        for i in range(T_t):
            _ = model(template, search, language)
        torch.cuda.synchronize()
...
if __name__ == "__main__":
    ...
    args = parse_args()
    '''update cfg'''
    yaml_fname = '/data/data6T/xls/projects/update_base_seamtrack/experiments/%s/%s.yaml' % (args.script, args.config)
    ...
    if args.script == "ostrack":
        ...
        # 32 * 40
        phrase_ids = torch.randint(0, 100, (bs, 40), dtype=torch.long)
        phrase_attnmask = torch.randn(bs, 40)
        # transfer to device
        model = model.to(device)
        template = [template.to(device),template.to(device),template.to(device)]
        search = [search.to(device)]
        language = ["this is a test language!"]


        merge_layer = cfg.MODEL.BACKBONE.MERGE_LAYER
        if merge_layer <= 0:
            evaluate_vit(model, template, search, language)
        else:
...
'''
overall macs is  48.529G
overall params is  92.713M
testing speed ...
The average overall latency is 13.26 ms
FPS is 75.40 fps
'''