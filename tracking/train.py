...
# 设置临时文件存储目录
# os.environ['TMPDIR'] = '/data/xls/PycharmProjects/cv/All-od-update-model/tmpdir'


os.environ['CUDA_VISIBLE_DEVICES'] = '5,4,3'
# os.environ['CUDA_VISIBLE_DEVICES'] = '5,4,3,1,0'
# 设置 NCCL 的超时时间
os.environ['NCCL_BLOCKING_WAIT'] = '1'
os.environ['CUDA_LAUNCH_BLOCKING'] = '1'

# 设置 NCCL 超时时间为 60 分钟
os.environ["NCCL_TIMEOUT_MS"] = "7200000"
# print(f"NCCL_BLOCKING_WAIT: {os.environ.get('NCCL_BLOCKING_WAIT')}")
# print(f"NCCL_TIMEOUT_MS: {os.environ.get('NCCL_TIMEOUT_MS')}")
...
def parse_args():
    ...
    parser.add_argument('--world-size', type=int, default=3, help='Number of processes participating in the job.')
...
def main():
    args = parse_args()
    if args.mode == "single":
        train_cmd = "python lib/train/run_training_all_in_one.py --script %s --config %s --save_dir %s --use_lmdb %d " \
        ...
        train_cmd = "torchrun --nproc_per_node %d --master_port %d /data/data6T/xls/projects/JUTrack/lib/train/run_training_all_in_one.py " \
        ...
        train_cmd = "python -m torch.distributed.launch --nproc_per_node %d --master_addr %s --master_port %d --nnodes %d --node_rank %d lib/train/run_training_all_in_one.py " \
                    "--script %s --config %s --save_dir %s --use_lmdb %d --script_prv %s --config_prv %s --use_wandb %d " \
                    "--distill %d --script_teacher %s --config_teacher %s" \
...
# python tracking/train.py --script ostrack --config vitb_256_mae_ce_32x4_ep300 --save_dir /home/dell/WorkSpace/Tracking_by_NL/All-in-One/output --mode multiple --nproc_per_node 4
